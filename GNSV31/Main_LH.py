...
class Main_Test(QtCore.QThread):
        ...
        try:
            ...
            while Prompt not in buff:
                ...
                if (steptime - runtime).seconds > Timeout:
                    self.Test_Fail(u"超时, %s不能找到" % Prompt)
                    break
            return log
        except Exception:
    ...
    def Check_Process(self):
        if int(self.GetMiddleStr(self.Send_Command("ps -A | grep -c kcpu"), "kcpu", "[")) < 2:
            ...
            self.Send_Command("./kcpu.sh start 50")
            self.Send_Command("cd /")
    ...
    def VersionCheck(self):
        ...
        s = self.Send_Command("vershow", '#', 60,30)
        if "failed" in s:
    ...
    def MAC_check(self):
        ...
        MAC = self.Send_Command("ifconfig",'#', 60,30)
        if "00:a0:c9:00:00:00" not in MAC:
            self.Test_Fail(u"MAC地址格式错误")

    #4G模块检测
    def Check4GModule(self):
        self.Test_Running(u"4G模块检测")
        if '1' not in self.Send_Command("lsusb | grep -c '2c7c'"):  # 4G模块检测
            self.Test_Fail(u"2c7c Module FAIL")
            at_4G = self.Send_Command("atget simcard")
            if 'Success' not in at_4G:
    ...
    def ShowTemperature(self):
        ...
        if float(Core0) > 105:
            self.Test_Fail(u"CPU Core 0 温度超过105℃,当前温度：%s℃" % Core0)
        Core1 = self.GetMiddleStr(str(temp), 'temp3_input:', "temp3_max:")
        if float(Core1) > 105:
            self.Test_Fail(u"CPU Core 1 温度超过105℃,当前温度：%s℃" % Core0)
    ...
    def Discrete(self):
        ...
        low = int(
            low[3] + low[7] + low[11] + low[15] + low[19] + low[23] + low[27] + low[31] + low[35] + low[39] + low[
                44] + low[49] + low[54])
        ...
        high = int(
            high[3] + high[7] + high[11] + high[15] + high[19] + high[23] + high[27] + high[31] + high[35] + high[
                39] + high[44] + high[49] + high[54])
        log1 = low + high
    ...
    def SSDCheck(self):
        ...
        if "8" in num1 and "4" in num2:
            ...
            if int(self.GetMiddleStr(self.Send_Command("ps -A | grep -c dd"), "dd", "[root")) < 2:
