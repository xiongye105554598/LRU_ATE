...
class Button(QtGui.QPushButton):
    ...
    def RightMenuShow(self):
        self.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)           #保持通用菜单设置
        self.customContextMenuRequested.connect(self.showContextMenu)    #右键点击弹出菜单并调用方法
        self.contextMenu = QtGui.QMenu(self)                             #创建菜单
        self.CWSU = self.contextMenu.addMenu("CWSU")                     #添加一级菜单
        self.BI7 = self.CWSU.addAction(u'老化')                           #添加二级菜单
        self.BI7.triggered.connect(lambda: self.Production(10))          #将动作与槽函数关联

    #右键点击调用函数
    def showContextMenu(self):
        #菜单显示前，将它移动到鼠标点击的位置
        self.contextMenu.exec_(QtGui.QCursor.pos())                      #在当前坐标下显示，但是菜单停止不走动，方案一
        #self.contextMenu.move(QtGui.QCursor.pos())                      #将菜单在当前位置显示，方案二
    ...
    def input_ip_address(self,message):
        ...
        if Input_IP_address !="":
            ...
            if re.match(match,Input_IP_address):        #匹配IP地址格式
    ...
    def Production(self,Value):
        ...
        self.Cell_SN()
        self.IP = self.input_ip_address(u"请输入 Cell " + str(self.ID) + u" 地址")       #输入设备IP
        Remote_IP = self.input_ip_address(u"请输入服务器IP地址，如果没有请不要输入")          #输入服务器IP
