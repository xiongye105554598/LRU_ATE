...
class Windows(QtGui.QWidget):
    def __init__( self,ID="", parent = None):
        ...
        self.new.name.setText("<font color=black><font size = 8>%s</font>" % u"CWAP")
        ...
        self.new.Select_PD.addItem("CWAP")

        self.new.Upgrade.setDisabled(1)         #设置升级不可用
        self.new.Modify_IP.setDisabled(1)       #设置修改IP不可用
        self.new.Shipout.setDisabled(1)         #设置出厂设置不可用
